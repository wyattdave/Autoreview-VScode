...
function CreateReview(
    ...
    sEnvironment
      ...
      keys.forEach((key) => {
          let value = oInput.properties.definition.triggers[key];
          sTrigger = key;
      ...
      aActions.forEach((item, index) => {
          ...
          if (JSON.stringify(item.runAfter) == "{}" || item.runAfter==undefined) {
          ...
          let aEnvironVarRaw = JSON.stringify(oTempItem).match(regExpEnviron);

          let aEnvironVar;
  
          let bEnvironVar = false;
          if (aEnvironVar) {
              aEnvironVar = aEnvironVarRaw.filter(
          ...
          if (!bEnvironVar) {
            let aEnvironVarRaw = JSON.stringify(oTempItem).match(regExpEnviron2);
              if (aEnvironVar) {
              aEnvironVar = aEnvironVarRaw.filter(
                  (object) => object != "@{parameters('$authentication')"
              );
